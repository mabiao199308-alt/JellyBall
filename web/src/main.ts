...
import { defaultDeathFxCfg, resolveDeathFxCfg } from "./death_fx";
import {
  BEST_STORAGE_KEY,
  CFG_DEFAULT_OVERRIDE_KEY,
  CFG_STORAGE_KEY,
  CODE_DEFAULT_SAVE_ENDPOINT,
  CODE_DEFAULT_SAVE_ENDPOINT_FALLBACK,
...
import {
  ...
  createJuiceSplat as createJuiceSplatRuntime,
  drawDeathFxState as drawDeathFxStateRuntime,
  spawnGroundJuiceSpread as spawnGroundJuiceSpreadRuntime,
  spawnJuiceBurstParticle as spawnJuiceBurstParticleRuntime,
  updateDeathFxState as updateDeathFxStateRuntime,
} from "./systems/death_fx_runtime";
import { applyGeneratedSlotCounters, pickGeneratedSlot } from "./systems/hazard_loop";
import { screenToWorldPoint as screenToWorldByCamera, toScreenPointFromEvent, toWorldPointFromEvent } from "./systems/input";
import { canHookCandidate, canTryAnchorHook, getAnchorHookHitRadius } from "./systems/hooks";
import { applyAirDrag, applyGravity, integrateBody } from "./systems/physics";
import { renderFpsHud, renderMeterHud } from "./systems/render";
import { drawAnchorsWorld, drawGearHazardsWorld, drawMovingTrackWorld } from "./systems/render_world";
import {
...
import { safeGetJSON, safeGetNumber, safeGetString, safeRemoveKeys, safeSetJSON, safeSetString } from "./systems/storage";
import { setTextStatus, syncPanelVisibility } from "./systems/ui";
import { runMainUpdateStep } from "./systems/update_loop";
import wallHitSfxUrl from "../Arrive_1.wav?url";
import deathPopSfxUrl from "../dead.mp3?url";
...
const canvas = mustEl<HTMLCanvasElement>("gameCanvas");
const ctx = canvas.getContext("2d");
if (!ctx) throw new Error("2D context unavailable");
const gameShell = document.getElementById("gameShell") as HTMLElement | null;
...
const debugToggleBtn = mustEl<HTMLButtonElement>("debugToggleBtn");
const deathFxDebugPanelBody = mustEl<HTMLElement>("deathFxDebugPanelBody");
const deathFxDebugPanel = mustEl<HTMLElement>("deathFxDebugPanel");
const deathFxDebugToggleBtn = mustEl<HTMLButtonElement>("deathFxDebugToggleBtn");
const deathFxPreviewBtn = mustEl<HTMLButtonElement>("deathFxPreviewBtn");
const deathFxPlaySfxBtn = mustEl<HTMLButtonElement>("deathFxPlaySfxBtn");
const deathFxDefaultBtn = mustEl<HTMLButtonElement>("deathFxDefaultBtn");
const jellyDebugPanelBody = mustEl<HTMLElement>("jellyDebugPanelBody");
const jellyDebugPanel = mustEl<HTMLElement>("jellyDebugPanel");
const jellyDebugToggleBtn = mustEl<HTMLButtonElement>("jellyDebugToggleBtn");
const jellyDefaultBtn = mustEl<HTMLButtonElement>("jellyDefaultBtn");
const jellyCfgStatus = mustEl<HTMLElement>("jellyCfgStatus");
const hazardDebugPanelBody = mustEl<HTMLElement>("hazardDebugPanelBody");
const hazardDebugPanel = mustEl<HTMLElement>("hazardDebugPanel");
const hazardDebugToggleBtn = mustEl<HTMLButtonElement>("hazardDebugToggleBtn");
const hazardSaveBtn = mustEl<HTMLButtonElement>("hazardSaveBtn");
const hazardSaveAsDefaultBtn = mustEl<HTMLButtonElement>("hazardSaveAsDefaultBtn");
const hazardSaveCodeBtn = mustEl<HTMLButtonElement>("hazardSaveCodeBtn");
const hazardDefaultBtn = mustEl<HTMLButtonElement>("hazardDefaultBtn");
const hazardCfgStatus = mustEl<HTMLElement>("hazardCfgStatus");
const toggleAdvancedBtn = mustEl<HTMLButtonElement>("toggleAdvancedBtn");
const saveCfgBtn = mustEl<HTMLButtonElement>("saveCfgBtn");
const saveAsDefaultBtn = mustEl<HTMLButtonElement>("saveAsDefaultBtn");
const copyCfgCodeBtn = mustEl<HTMLButtonElement>("copyCfgCodeBtn");
const defaultCfgBtn = mustEl<HTMLButtonElement>("defaultCfgBtn");
...
const WALL_HIT_SFX_CANDIDATES = [wallHitSfxUrl];
...
const DEATH_POP_SFX_CANDIDATES = [deathPopSfxUrl];
...
const GAME_OVER_OVERLAY_FADE_DURATION = 0.24;
const GAME_OVER_PANEL_REVEAL_DURATION = 0.32;
const GAME_OVER_TEXT_REVEAL_DURATION = 0.18;
...
const hazardParamDefs = [
  { key: "baseTrackUnlockMeters", label: "基础轨道解锁", min: 0, max: 220, step: 1 },
  { key: "gearUnlockMeters", label: "齿轮解锁米数", min: 0, max: 260, step: 1 },
  { key: "damageTrackUnlockMeters", label: "伤害轨道解锁", min: 0, max: 320, step: 1 },
  { key: "baseTrackSlotInterval", label: "基础轨道槽位", min: 1, max: 10, step: 1 },
  { key: "baseTrackSpawnChance", label: "基础轨道概率", min: 0.05, max: 1, step: 0.01 },
  { key: "baseTrackSpawnChanceMax", label: "基础轨道最高", min: 0.05, max: 1, step: 0.01 },
  { key: "damageTrackSlotInterval", label: "伤害轨道槽位", min: 1, max: 10, step: 1 },
  { key: "damageTrackSpawnChance", label: "伤害轨道概率", min: 0.05, max: 1, step: 0.01 },
  { key: "damageTrackSpawnChanceMax", label: "伤害轨道最高", min: 0.05, max: 1, step: 0.01 },
...
const ballVisualCfg = { ...defaultBallVisualCfg };
const jellyLayerVisibility = { ...(defaultLayerVisibility || {}) };
const deathFxCfg = { ...defaultDeathFxCfg };
...
function loadCfgFromStorage() {
  ...
  const parsed = safeGetJSON(CFG_STORAGE_KEY);
...
function applyCfgDefaultOverrideFromStorage() {
  const parsed = safeGetJSON(CFG_DEFAULT_OVERRIDE_KEY);
...
function applyHazardDefaultOverrideFromStorage() {
  const parsed = safeGetJSON(HAZARD_DEFAULT_OVERRIDE_KEY);
  if (!parsed || typeof parsed !== "object") return;
...
function saveCfgToStorage() {
  ...
  safeSetJSON(CFG_STORAGE_KEY, payload);
}

function loadJellyCfgFromStorage() {
  const parsed = safeGetJSON(JELLY_CFG_STORAGE_KEY);
  if (!parsed || typeof parsed !== "object") return;
...
function saveJellyCfgToStorage() {
  ...
  safeSetJSON(JELLY_CFG_STORAGE_KEY, payload);
}
...
function applyJellyLayerVisibilityFromStorage() {
  try {
    ...
    const parsed = safeGetJSON(JELLY_LAYER_VISIBILITY_KEY);
    if (!parsed || typeof parsed !== "object") return;
...
function loadHazardCfgFromStorage() {
  const parsed = safeGetJSON(HAZARD_CFG_STORAGE_KEY);
  if (!parsed || typeof parsed !== "object") return;
  if (Number.isFinite(Number(parsed.trackUnlockMeters))) {
    hazardCfg.baseTrackUnlockMeters = Number(parsed.trackUnlockMeters);
  }
  if (Number.isFinite(Number(parsed.trackGearUnlockMeters))) {
    hazardCfg.damageTrackUnlockMeters = Number(parsed.trackGearUnlockMeters);
  }
  if (Number.isFinite(Number(parsed.trackSlotInterval))) {
    hazardCfg.baseTrackSlotInterval = Number(parsed.trackSlotInterval);
  }
  if (Number.isFinite(Number(parsed.trackSpawnChance))) {
    hazardCfg.baseTrackSpawnChance = Number(parsed.trackSpawnChance);
  }
  if (Number.isFinite(Number(parsed.trackSpawnChanceMax))) {
    hazardCfg.baseTrackSpawnChanceMax = Number(parsed.trackSpawnChanceMax);
  }
  if (Number.isFinite(Number(parsed.trackGearSpawnRatio))) {
    const ratio = clamp(Number(parsed.trackGearSpawnRatio), 0, 1);
    hazardCfg.damageTrackSpawnChance = Math.max(0.05, ratio);
    hazardCfg.damageTrackSpawnChanceMax = Math.max(hazardCfg.damageTrackSpawnChance, ratio);
  }
  for (const def of hazardParamDefs) {
    const val = Number(parsed[def.key]);
...
function saveHazardCfgToStorage() {
  ...
  safeSetJSON(HAZARD_CFG_STORAGE_KEY, payload);
}
...
function loadBestMeters() {
  return safeGetNumber(BEST_STORAGE_KEY, 0);
}

function saveBestMeters() {
  safeSetString(BEST_STORAGE_KEY, String(world.bestMeters));
}

function loadTutorialSeen() {
  return safeGetString(TUTORIAL_SEEN_STORAGE_KEY) === "1";
}
...
function markTutorialSeen() {
  ...
  tutorialSeen = true;
  safeSetString(TUTORIAL_SEEN_STORAGE_KEY, "1");
}
...
function setStatus(text: string, _error = false) {
  setTextStatus(cfgStatus, text);
}

function setDeathFxStatus(text) {
  setTextStatus(deathFxCfgStatus, text);
}

function setJellyStatus(text) {
  setTextStatus(jellyCfgStatus, text);
}

function setHazardStatus(text) {
  setTextStatus(hazardCfgStatus, text);
}

function syncDebugPanelVisibility() {
  syncPanelVisibility(debugPanel, debugToggleBtn, debugPanelVisible, "隐藏调试", "显示调试");
}

function syncDeathFxPanelVisibility() {
  syncPanelVisibility(
    deathFxDebugPanel,
    deathFxDebugToggleBtn,
    deathFxPanelVisible,
    "隐藏死亡特效调试",
    "显示死亡特效调试",
  );
...
function normalizeDeathFxCfg() {
  Object.assign(deathFxCfg, resolveDeathFxCfg(deathFxCfg));
...
function playWallHitSfx(impact: number, options: { force?: boolean } = {}) {
...
function createAnchor(
  ...
  y: number,
  radius = 8,
  options: { id?: number; ignoreEarlyBlueRule?: boolean; isRed?: boolean; spawnAnimStartSec?: number } = {},
) {
...
function getFixedAnchorXByCursor(cursor) {
  return calcFixedAnchorXByCursor(cursor, ANCHOR_X_RATIOS, world.w, cfg.anchorSidePadding);
}

function getAnchorMaxStepX() {
  return calcAnchorMaxStepX(world.w, cfg.maxStretch);
}

function getDynamicAnchorSpacingRange() {
  return calcDynamicAnchorSpacingRange(
    world.runMeters,
    cfg.anchorSpacingMin,
    cfg.anchorSpacingMax,
    ANCHOR_DIFFICULTY_START_METERS,
    ANCHOR_DIFFICULTY_FULL_METERS,
    ANCHOR_SPACING_BONUS_MIN,
    ANCHOR_SPACING_BONUS_MAX,
  );
}

function getDynamicRedAnchorChance() {
  return calcDynamicRedAnchorChance(world.runMeters, RED_ANCHOR_CHANCE_FULL_METERS, RED_ANCHOR_CHANCE_START, RED_ANCHOR_CHANCE_END);
}

function getRandomizedAnchorXByCursor(cursor) {
  return calcRandomizedAnchorXByCursor(cursor, ANCHOR_X_RATIOS, world.w, cfg.anchorSidePadding, rand);
}

function getRandomizedAnchorXBySide(side) {
  return calcRandomizedAnchorXBySide(side, world.w, cfg.anchorSidePadding, rand, randInt);
}
...
function canSpawnMovingTrackFromGenerator() {
...
function addGeneratedSlotAbove() {
  ...
  if (selected === "baseTrack") {
    spawnMovingTrackAtY(world.movingTrack, y, "pin");
    world.generatedTopY = y;
    applyGeneratedSlotCounters(selected, world);
    return;
  }

  if (selected === "damageTrack") {
    spawnMovingTrackAtY(world.movingTrack, y, "gear");
    world.generatedTopY = y;
    applyGeneratedSlotCounters(selected, world);
    return;
  }
...
function resetRun() {
  ...
  world.damageTrackSlotsSinceSpawn = 0;
  world.gearSlotsSinceSpawn = 0;
  world.pendingSafeAnchorSide = 0;
...
function toWorldPoint(e) {
  return toWorldPointFromEvent(canvas, world.cameraX, world.cameraY, e);
}

function screenToWorldPoint(p) {
  return screenToWorldByCamera(world.cameraX, world.cameraY, p);
}

function toScreenPoint(e) {
  return toScreenPointFromEvent(canvas, e);
}
...
function applyFreeFlightPhysics(dt) {
  const b = world.ball;
  applyGravity(b, cfg.gravity, dt);
  applyAirDrag(b, cfg.airDrag);
  integrateBody(b, dt);
  collideBounds();
}
...
function checkAnchorHook() {
  if (!canTryAnchorHook(world)) return;
  const b = world.ball;
  for (const a of world.anchors) {
    if (!canHookCandidate(a, world)) continue;
    const d = Math.hypot(b.x - a.x, b.y - a.y);
    if (d <= getAnchorHookHitRadius(cfg, a)) {
      hookToAnchor(a);
      return;
    }
  }
...
function checkMovingTrackHit() {
  ...
  if (t.mode === "pin") {
    ...
    const hitRadius = getAnchorHookHitRadius(cfg, t.pinAnchor);
...
function updateMeterHud() {
  renderMeterHud(meterDisplayEl, world.runMeters);
}

function updateFpsHud() {
  renderFpsHud(fpsDisplayEl, world.fps);
}

function updateCamera(dt) {
  updateCameraState(world as any, cfg as any, dt, clamp);
}

function getJuicePalette() {
  return buildJuicePaletteRuntime(ballVisualCfg);
}

function spawnJuiceBurstParticle(fx, originX, originY, palette, big = false) {
  spawnJuiceBurstParticleRuntime(fx, originX, originY, palette, deathFxCfg, rand, randInt, big);
}

function createJuiceSplat(x: number, y: number, color: string, alpha: number, scale = 1, options: Record<string, any> = {}) {
  return createJuiceSplatRuntime(x, y, color, alpha, scale, options, rand, randInt);
}

function spawnGroundJuiceSpread(fx: any, centerX: number, floorY: number, palette: string[], intensity = 1, options: Record<string, any> = {}) {
  spawnGroundJuiceSpreadRuntime(fx, centerX, floorY, palette, world.w, world.h, clamp, rand, randInt, intensity, options);
}

function startDeathFx(originX: number, originY: number, options: { previewOnly?: boolean } = {}) {
...
function updateDeathFx(dt) {
  ...
  updateDeathFxStateRuntime(fx, dt, {
    worldW: world.w,
    worldH: world.h,
    deathFxCfg,
    rand,
  ...
  if (!fx.active) {
    ...
    if (!fx.previewOnly) {
      ...
      world.gameOverStartSec = world.timeSec;
    }
  }
}
...
function update(dt) {
  runMainUpdateStep(world, cfg, dt, {
    syncTutorialOverlay,
    updateBestFireworks,
    updateGear,
    updateMovingTrack,
...
function drawMovingTrack() {
  drawMovingTrackWorld(ctx, {
    TRACK_ENABLED,
    world,
    toScreenX,
    toScreenY,
    getAnchorFuseState,
    getRedAnchorSpawnAnimState,
    getAnchorVisualStyle,
    GEAR_TEETH,
  });
}
...
function getBallVisualRadius() {
  return getVisualRadius(cfg.ballRadius, ballVisualCfg);
}
...
function drawAnchors() {
  drawAnchorsWorld(ctx, {
    anchors: world.anchors,
    activeAnchor: world.activeAnchor,
    world,
    toScreenX,
    toScreenY,
    getAnchorFuseState,
...
function drawDeathFx() {
  drawDeathFxStateRuntime(ctx, world.deathFx, clamp01, lerp);
}
...
function drawGameOver() {
  if (world.state !== "gameover") return;
  if (world.gameOverStartSec < 0) world.gameOverStartSec = world.timeSec;
  const centerX = world.w * 0.5;
  ...
  const elapsed = Math.max(0, world.timeSec - world.gameOverStartSec);
  const overlayT = easeOutCubic(clamp01(elapsed / GAME_OVER_OVERLAY_FADE_DURATION));
  const panelT = clamp01(elapsed / GAME_OVER_PANEL_REVEAL_DURATION);
  const panelScale = lerp(0.94, 1, easeOutCubic(panelT));
  const panelRise = lerp(34, 0, easeOutCubic(panelT));
  const panelAlpha = easeOutCubic(clamp01((elapsed - 0.02) / 0.22));
  const animatedPanelCenterY = panelY + panelH * 0.5 + panelRise;
  const animatedPanelY = animatedPanelCenterY - panelH * 0.5;
  const titleBarInset = 10;
  const titleBarX = panelX + titleBarInset;
  const titleBarY = animatedPanelY + titleBarInset;
  const titleBarW = panelW - titleBarInset * 2;
  const titleBarH = panelH * 0.32;
  const titleReveal = easeOutCubic(clamp01((elapsed - 0.04) / GAME_OVER_TEXT_REVEAL_DURATION));
  const score1Reveal = easeOutCubic(clamp01((elapsed - 0.1) / GAME_OVER_TEXT_REVEAL_DURATION));
  const score2Reveal = easeOutCubic(clamp01((elapsed - 0.14) / GAME_OVER_TEXT_REVEAL_DURATION));
  const dividerReveal = easeOutCubic(clamp01((elapsed - 0.12) / GAME_OVER_TEXT_REVEAL_DURATION));
  const hintReveal = easeOutCubic(clamp01((elapsed - 0.18) / GAME_OVER_TEXT_REVEAL_DURATION));
  ...
  ctx.globalAlpha = overlayT;
  ctx.fillStyle = overlay;
  ctx.fillRect(0, 0, world.w, world.h);
  ctx.globalAlpha = 1;

  const rippleT = clamp01(elapsed / 0.6);
  if (rippleT < 1) {
    const rippleRadius = lerp(panelW * 0.25, Math.max(world.w, world.h) * 0.7, easeOutCubic(rippleT));
    const rippleAlpha = (1 - rippleT) * 0.26;
    ctx.strokeStyle = `rgba(189, 227, 255, ${rippleAlpha})`;
    ctx.lineWidth = lerp(12, 2, rippleT);
    ctx.beginPath();
    ctx.arc(centerX, animatedPanelCenterY, rippleRadius, 0, Math.PI * 2);
    ctx.stroke();
  }
  ...
  for (const bubble of GAME_OVER_BUBBLES) {
    ...
    const bubbleReveal = lerp(0.4, 1, overlayT);

    const ring = ctx.createRadialGradient(x, y, radius * 0.24, x, y, radius);
    ring.addColorStop(0, `rgba(194, 224, 251, ${0.95 * bubbleReveal})`);
    ring.addColorStop(0.72, `rgba(157, 197, 233, ${0.46 * bubbleReveal})`);
  ...
  ctx.save();
  ctx.translate(centerX, animatedPanelCenterY);
  ctx.scale(panelScale, panelScale);
  ctx.translate(-centerX, -animatedPanelCenterY);
  ctx.globalAlpha = panelAlpha;

  roundedRectPath(ctx, panelX, animatedPanelY, panelW, panelH, 34);
  const panelGrad = ctx.createLinearGradient(0, animatedPanelY, 0, animatedPanelY + panelH);
  ...
  ctx.fillRect(panelX, animatedPanelY, panelW, panelH);

  const coolGlow = ctx.createRadialGradient(
    panelX + panelW * 0.84,
    animatedPanelY + panelH * 0.1,
    0,
    panelX + panelW * 0.84,
    animatedPanelY + panelH * 0.1,
    panelW * 0.68,
  ...
  ctx.textAlign = "left";
  const titleCenterY = titleBarY + titleBarH * 0.5 + Math.max(4, titleSize * 0.08) + (1 - titleReveal) * 11;
  ctx.globalAlpha = Math.max(0.0001, titleReveal);
  ...
  ctx.textAlign = "center";
  const scoreBaseY = animatedPanelY + panelH * 0.56;
  ctx.globalAlpha = Math.max(0.0001, score1Reveal);
  ctx.fillText(`本局 ${Math.round(world.runMeters)}m`, centerX, scoreBaseY + (1 - score1Reveal) * 10);
  ctx.globalAlpha = Math.max(0.0001, score2Reveal);
  ctx.fillText(`最高 ${Math.round(world.bestMeters)}m`, centerX, animatedPanelY + panelH * 0.76 + (1 - score2Reveal) * 12);

  ctx.strokeStyle = "rgba(237, 246, 255, 0.55)";
  ctx.lineWidth = 1.4;
  ctx.globalAlpha = Math.max(0.0001, dividerReveal);
  ctx.beginPath();
  ctx.moveTo(centerX - panelW * 0.24, animatedPanelY + panelH * 0.66);
  ctx.lineTo(centerX + panelW * 0.24, animatedPanelY + panelH * 0.66);
  ctx.stroke();
  ctx.restore();

  const hintPulse = 0.5 + Math.sin(world.timeSec * 3.6) * 0.5;
  const hintAlpha = lerp(0.2, 0.98, hintPulse) * hintReveal;
  const hintY = world.h * 0.9;
  ctx.shadowColor = `rgba(151, 218, 255, ${(0.18 + hintPulse * 0.34) * hintReveal})`;
  ctx.shadowBlur = (10 + hintPulse * 16) * hintReveal;
  ctx.shadowOffsetY = 0;
  ctx.fillStyle = `rgba(236, 246, 255, ${hintAlpha})`;
  ctx.font = uiFont(hintSize, 600);
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("点击任意位置重新挑战", centerX, hintY + (1 - hintReveal) * 8);
  ctx.restore();
}
...
if (saveAsDefaultBtn) {
  saveAsDefaultBtn.addEventListener("click", async () => {
    ...
    safeSetJSON(CFG_DEFAULT_OVERRIDE_KEY, payload);
...
if (deathFxDefaultBtn) {
  deathFxDefaultBtn.addEventListener("click", () => {
    Object.assign(deathFxCfg, defaultDeathFxCfg);
    normalizeDeathFxCfg();
    syncDeathFxPanelFromCfg();
...
if (hazardSaveAsDefaultBtn) {
  hazardSaveAsDefaultBtn.addEventListener("click", async () => {
    ...
    safeSetJSON(HAZARD_DEFAULT_OVERRIDE_KEY, payload);
